  ...
  return (
    <div style={styles.loginContainer}>
      ...
      <div style={styles.loginCard}>
        ...
        <button style={styles.googleButton} onClick={onLogin}>
...
export default function App() {
  ...
  useEffect(() => {
    getRedirectResult(auth)
      .then((result) => {
        if (result?.user) console.log("✅ Login exitoso:", result.user.email);
      })
      .catch((err) => {
  ...
  const handleLogin = () => {
    ...
    signInWithRedirect(auth, provider).catch((err) => {
      setError(`Error al redirigir: ${err.message}`);
    });
  };
  ...
  return user ? <MapScreen user={user} onLogout={handleLogout} /> : <LoginScreen onLogin={handleLogin} error={error} />;
}
